...
void SENSORS_MeshInit(sensors_MeshInitTypeDef* meshInit)
{
	...
	/* test input parameters */
	assert_param(IS_MEMORY_IN_COMMON_RAM(sensorsInit->initialMeshKey, 32));
	assert_param(IS_MEMORY_IN_COMMON_RAM(sensorsInit->initialMeshVal, 2));
	assert_param(SENSORS_IS_MESH_CLK_DIV_VALID(sensorsInit->meshClkDiv));
