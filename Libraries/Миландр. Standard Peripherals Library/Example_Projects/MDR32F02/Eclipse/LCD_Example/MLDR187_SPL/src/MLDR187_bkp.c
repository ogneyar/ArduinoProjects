...
void BKP_GetDateTimeBCD(RTC_DateTime* dateTime)
{
	...
	TR = MDR_BKP->TR;
	DR = MDR_BKP->DR;
	if (TR != MDR_BKP->TR) {// race condition
		TR = MDR_BKP->TR;
		DR = MDR_BKP->DR;
	}
	BKP_GetDateTimeReg(DR, TR, dateTime);
}
...
void BKP_GetDateTimeBCDTimestamp(bkpTimestamp tsNum, RTC_DateTime* dateTime)
{
	...
	if (tsNum == bkpTimestamp1) {
		TR = MDR_BKP->TSTR1;
		DR = MDR_BKP->TSDR1;
		if (TR != MDR_BKP->TSTR1) {// race condition
			TR = MDR_BKP->TSTR1;
			DR = MDR_BKP->TSDR1;    
		}
		...
		TR = MDR_BKP->TSTR2;
		DR = MDR_BKP->TSDR2;
		if (TR != MDR_BKP->TSTR2) {// race condition
			TR = MDR_BKP->TSTR2;
			DR = MDR_BKP->TSDR2;    
		}
	} else {
		TR = MDR_BKP->TSTR3;
		DR = MDR_BKP->TSDR3;
		if (TR != MDR_BKP->TSTR3) {// race condition
			TR = MDR_BKP->TSTR3;
			DR = MDR_BKP->TSDR3;    
		}
	}
...
void BKP_SetAlarm(bkpAlarm alrm, BKP_AlarmInitTypeDef* initStruct)
{
	...
	assert_param(IS_FLAG_STATUS(initStruct->pmFlag));
	if ((initStruct->pmFlag) != RESET) {
	    assert_param(BKP_IS_BCD_PM_HOUR(initStruct->hours));
	}
