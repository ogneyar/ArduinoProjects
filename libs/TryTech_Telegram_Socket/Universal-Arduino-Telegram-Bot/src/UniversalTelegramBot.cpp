...
bool UniversalTelegramBot::processResult(JsonObject &result, int messageIndex) {
  ...
  if (last_message_received != update_id) {
    ...
    if (result.containsKey("message")) {
      ...
      messages[messageIndex].message_id = message["message_id"].as<String>();
      ...
      messages[messageIndex].message_id = message["message"]["message_id"].as<String>();
...
//TryTech method "editMessageText"
bool UniversalTelegramBot::editMessageText(String chat_id, String message_id, String text,
                                       String parse_mode) {
  ...
  return editPostMessageText(payload);
}

//TryTech method "deleteMessage"
bool UniversalTelegramBot::deleteMessage(String chat_id, String message_id) {
  ...
  return deletePostMessage(payload);
}
...
//TT
bool UniversalTelegramBot::editPostMessageText(JsonObject &payload) {

  bool sent = false;
  if (_debug)
    Serial.println(F("EDIT Post Message Text"));
  long sttime = millis();

  if (payload.containsKey("text")) {
    while (millis() < sttime + 8000) { // loop for a while to send the message
      String command = "bot" + _token + "/editMessageText";
      String response = sendPostToTelegram(command, payload);
      if (_debug)
        Serial.println(response);
      sent = checkForOkResponse(response);
...
//TT
bool UniversalTelegramBot::deletePostMessage(JsonObject &payload) {

  bool sent = false;
  if (_debug)
    Serial.println(F("DELETE Post Message Text"));
  long sttime = millis();

  if (payload.containsKey("chat_id")) {
    while (millis() < sttime + 8000) { // loop for a while to send the message
      String command = "bot" + _token + "/deleteMessage";
      String response = sendPostToTelegram(command, payload);
      if (_debug)
        Serial.println(response);
      sent = checkForOkResponse(response);
