 ...
 *@Note
  ...
  and endpoints 3 and 5 are directly copied and inverted for upload.
...
int main(void)
{
    ...
    while(1)
    {
	    ...
	    if( USBHD_DevEnumStatus )
	    {
	        ...
	        {
	            ret = USBHD_Endp_DataUp(DEF_UEP2, &Data_Buffer[(RingBuffer_Comm.DealPtr) * DEF_USBD_FS_PACK_SIZE], RingBuffer_Comm.PackLen[RingBuffer_Comm.DealPtr], DEF_UEP_CPY_LOAD);
	            if(ret == 0)
	            {
