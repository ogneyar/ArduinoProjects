...
uint8_t   CreateDirectory(void)
{
    ...
    else
    {
        ...
        UpDirCluster = CH103vStartCluster;                             //保存上级目录的起始簇号
    }
    if(i == ERR_SUCCESS)                                               //成功获取上级目录的起始簇号
    {
        i = CH103FileOpen();                                           //打开本级子目录
        if(i == ERR_SUCCESS)
        {
        ...
        {
            i = CH103FileCreate();                                     //以创建文件的方法创建目录
            if(i == ERR_SUCCESS)
            {
                if(pDISK_FAT_BUF == pDISK_BASE_BUF)
                {
                    memset(pDISK_FAT_BUF, 0, CH103vStartCluster);   //如果FILE_DATA_BUF与DISK_BASE_BUF合用则必须清除磁盘缓冲区
                }
                DirXramBuf = pDISK_FAT_BUF;                            //文件数据缓冲区
                DirConstData = (uint8_t *)".          \x10\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x21\x30\x0\x0\x0\x0\x0\x0..         \x10\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x21\x30\x0\x0\x0\x0\x0\x0";
                ...
                *(pDISK_FAT_BUF + 0x1A) = ((uint8_t*)&CH103vStartCluster)[3];//自身的起始簇号
                *(pDISK_FAT_BUF + 0x1B) = ((uint8_t*)&CH103vStartCluster)[2];
                *(pDISK_FAT_BUF + 0x14) = ((uint8_t*)&CH103vStartCluster)[1];
                *(pDISK_FAT_BUF + 0x15) = ((uint8_t*)&CH103vStartCluster)[0];
...
//              for ( count = 0x40; count != CHRV3vSectorSizeH*256; count ++ )
//                {
//                    /* 清空目录区剩余部分 */
//                  *DirXramBuf = 0;
//                  DirXramBuf ++;
//              }
                ...
                if(i == ERR_SUCCESS)
                {
                    ...
                    for(j = 1; j != CH103vSecPerClus; j ++)
                    {
                        if(pDISK_FAT_BUF == pDISK_BASE_BUF)
                        {
                            memset(pDISK_FAT_BUF, 0, CH103vSectorSize); //如果FILE_DATA_BUF与DISK_BASE_BUF合用则必须清除磁盘缓冲区
                        }
                        mCmdParam.Write.mSectorCount = 1;
                        mCmdParam.Write.mDataBuffer = pDISK_FAT_BUF;         //指向文件数据缓冲区的起始地址
                        i = CH103FileWrite();                                //清空目录的剩余扇区
                        if(i != ERR_SUCCESS)
                        {
                            break;
                        }
                    }
                    if(j == CH103vSecPerClus)                                 //成功清空目录
                    {
...
void UDisk_USBH_CreatDirectory( void )
{
    ...
    if( ( CH103DiskStatus >= DISK_MOUNTED )&&( UDisk_Opeation_Flag == 1 ) )
    {
        ...
        strcpy( (char *)mCmdParam.Create.mPathName, "/YEAR2004" );     //目录名,该目录建在根目录下
        ret = CreateDirectory( );                                      //新建或者打开目录
        mStopIfError( ret );
        /* 目录新建或者打开成功,下面在这个子目录中新建一个演示文件 */
        printf( "Create New File /YEAR2004/DEMO2004.TXT \r\n" );
        strcpy( (char *)mCmdParam.Create.mPathName, "/YEAR2004/DEMO2004.TXT" );//文件名
        ret = CH103FileCreate( );                                      //新建文件并打开,如果文件已经存在则先删除后再新建
        mStopIfError( ret );
        printf( "Write some data to file DEMO2004.TXT \r\n" );
        i = sprintf( (char *)Com_Buffer, "演示文件\xd\xa" );
        mCmdParam.ByteWrite.mByteCount = i;                            //指定本次写入的字节数,单次读写的长度不能超过MAX_BYTE_IO
        mCmdParam.ByteWrite.mByteBuffer = Com_Buffer;                  //指向缓冲区
        ret = CH103ByteWrite( );                                       //以字节为单位向文件写入数据,单次读写的长度不能超过MAX_BYTE_IO
        mStopIfError( ret );
        ...
        strcpy( (char *)mCmdParam.Create.mPathName, "/YEAR2004/MONTH05" );    //目录名,该目录建在YEAR2004子目录下,YEAR2004目录必须事先存在
        ret = CreateDirectory( );                                     //新建或者打开目录
        mStopIfError( ret );
        printf( "Close\r\n" );
        mCmdParam.Close.mUpdateLen = 0;                               //对于目录不需要自动更新文件长度
        ret = CH103FileClose( );                                      //关闭目录,目录不需要关闭,关闭只是为了防止下面误操作
        mStopIfError( ret );
    }
}
