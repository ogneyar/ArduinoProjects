...
static volatile uint32_t Counter_Error_DMA = 0;
...
static struct USART_name husart1;
...
{	
  ...
  /* Initialize all configured peripherals */
	GPIO_Init();	
	/* Initialize Analog to Digital Compare */
	//ADC_Init();
	/* Initialize USART */
	UART_Init();
  ...
  while (1)
  {
		...
		/*----------Transmit array byte----------*/
		//static uint8_t A[6] = { 4, 8, 15, 16, 23, 42 };
...
void UART_Init(void) {
	...
	MODIFY_REG(GPIOA->CRH, GPIO_CRH_CNF9, GPIO_CRH_CNF9_1); // 0b10
	SET_BIT(GPIOA->CRH, GPIO_CRH_MODE9); // 0b11
	// PA10
	//Rx - Input floating
	MODIFY_REG(GPIOA->CRH, GPIO_CRH_CNF10, GPIO_CRH_CNF10_0); // 0b01
	CLEAR_BIT(GPIOA->CRH, GPIO_CRH_MODE10); // 0b00
	
	// USART1 enabled
	SET_BIT(RCC->APB2ENR, RCC_APB2ENR_USART1EN);
		
	MODIFY_REG(USART1->BRR, USART_BRR_DIV_Mantissa, 0x27 << /*USART_BRR_DIV_Mantissa_Pos*/ 4);
	MODIFY_REG(USART1->BRR, USART_BRR_DIV_Fraction, 0x1 << /*USART_BRR_DIV_Fraction_Pos*/ 0);
		
	//27.6.4 Control register 1(USART_CR1)(??. ??? 821 Reference Manual)
...
void ADC_Init(void) {	
	...
	SET_BIT(ADC1->CR1, ADC_CR1_EOCIE); // EOCIE: Interrupt enable for EOC // 1: EOC interrupt enabled (NVIC_EnableIRQ(ADC1_2_IRQn))
	//CLEAR_BIT(ADC1->CR1, ADC_CR1_EOCIE); // EOCIE: Interrupt enable for EOC // 0: EOC interrupt disabled
	...
	SET_BIT(ADC1->CR1, ADC_CR1_SCAN); // SCAN: Scan mode // 1: Scan mode enabled
	...
	// !!! necessarily CLEAR_BIT or SET_BIT - ADC_CR2_ALIGN
	...
	MODIFY_REG(ADC1->SQR3, ADC_SQR3_SQ2, ADC_SQR3_SQ2_0 | ADC_SQR3_SQ2_4); // hz (17 = 0b10001)
...
/* Clock configuration register (RCC_CFGR) */
	...
	// HPRE: AHB prescaler
	MODIFY_REG(RCC->CFGR, RCC_CFGR_HPRE, RCC_CFGR_HPRE_DIV1); // 0000: SYSCLK not divided
/* --------------------------------------- */

/* Flash access control register (FLASH_ACR) */
	// LATENCY: Latency
	MODIFY_REG(FLASH->ACR, (unsigned int)FLASH_ACR_LATENCY, FLASH_ACR_LATENCY_2); // 010: Two wait states, if 48 MHz < SYSCLK <= 72 MHz
...
/* Clock configuration register (RCC_CFGR) */
	...
	MODIFY_REG(RCC->CFGR, RCC_CFGR_PPRE1, RCC_CFGR_PPRE1_DIV2); // 100: HCLK divided by 2 // PCLK1 max 36MHz
	// PPRE2: APB high-speed prescaler (APB2)
	MODIFY_REG(RCC->CFGR, RCC_CFGR_PPRE2, RCC_CFGR_PPRE2_DIV1); //000: HCLK not divided // 72MHz
	// ADCPRE: ADC prescaler
	MODIFY_REG(RCC->CFGR, RCC_CFGR_ADCPRE, RCC_CFGR_ADCPRE_DIV6); // 10: PCLK2 divided by 6 // 72/6 = 12Mhz (< 14 ok)
	// PLLSRC: PLL entry clock source
	SET_BIT(RCC->CFGR, RCC_CFGR_PLLSRC); // 1: HSE oscillator clock selected as PLL input clock
	// PLLXTPRE: HSE divider for PLL entry
	//CLEAR_BIT(RCC->CFGR, RCC_CFGR_PLLXTPRE_HSE); // 0: HSE clock not divided
	// PLLMUL: PLL multiplication factor
	MODIFY_REG(RCC->CFGR, RCC_CFGR_PLLMULL, RCC_CFGR_PLLMULL9); // 0111: PLL input clock x 9 (8*9 = 72)
	// USBPRE: USB prescaler
	//CLEAR_BIT(RCC->CFGR, RCC_CFGR_USBPRE); // 0: PLL clock is divided by 1.5 (72 / 1.5 = 48 MHz)
	// MCO: Microcontroller clock output
	MODIFY_REG(RCC->CFGR, RCC_CFGR_MCO, RCC_CFGR_MCO_PLL); // 111: PLL clock divided by 2 selected
/* --------------------------------------- */
...
/* Clock control register (RCC_CR) */	
	...
	// SW: System clock switch
	MODIFY_REG(RCC->CFGR, RCC_CFGR_SW, RCC_CFGR_SW_PLL); // 10: PLL selected as system clock
...
void ADC1_2_IRQHandler(void) {
	...
	if (READ_BIT(ADC1->SR, ADC_SR_EOC)) { // Bit 1 EOC: End of conversion
		ADC1->DR; // read channel, for reset flag
	}
...
void USART1_IRQHandler(void) {
	...
	if (READ_BIT(USART1->SR, USART_SR_IDLE)) {
		//???? ???????? ???? IDLE
		USART1->DR; // read channel, for reset flag
		husart1.rx_len = husart1.rx_counter; //??????, ??????? ???? ????????
		CMSIS_USART_Transmit(USART1, husart1.rx_buffer, husart1.rx_counter); //???????? ? ???? ??, ??? ????????? ??? ????????.
