...
bool I2C_Data_Transmit(I2C_TypeDef *I2C, uint8_t Adress_Device, uint8_t* data, uint16_t Size_data, uint32_t Timeout_ms) {
	...
	if (READ_BIT(I2C->SR2, I2C_SR2_BUSY)) {
		...
		if (READ_BIT(I2C->SR2, I2C_SR2_MSL)) {
			//???? ????? ??????, ??? ?? ? ???????
			SET_BIT(I2C->CR1, I2C_CR1_STOP); //???????? ?????? STOP
		} 
	...
	//????????!
	/* ??? I2C_SR1_SB ????????? ?????????? ????? ?????? ???????? SR1 ? ??????????? ??????? ? ??????? DR ??? ????? PE=0*/
	I2C->SR1;
	I2C->DR = (Adress_Device << 1); //????? + Write
		...
		for (uint16_t i = 0; i < Size_data; i++) {
			...
			while (READ_BIT(I2C->SR1, I2C_SR1_TXE) == 0) {
				...
				if ((READ_BIT(I2C->SR1, I2C_SR1_AF) == 1)) {
					//???? ?????????? ?? ??????????, ???????? 1 ? I2C_SR1_AF 
					SET_BIT(I2C->CR1, I2C_CR1_STOP); //?????????????
					CLEAR_BIT(I2C->SR1, I2C_SR1_AF); //?????????? ??? AF
					return false;
				}
			} 
		}
...
bool I2C_Data_Receive(I2C_TypeDef *I2C, uint8_t Adress_Device, uint8_t* data, uint16_t Size_data, uint32_t Timeout_ms) {
	...
	if (READ_BIT(I2C->SR1, I2C_SR1_ADDR)) {
		...
		for (uint16_t i = 0; i < Size_data; i++) {
			if (i < Size_data - 1) {
				SET_BIT(I2C->CR1, I2C_CR1_ACK); //???? ?? ????? ??????? ????????? ????, ?? ?????????? ACK
				...
				*(data + i) = I2C->DR; //?????? ?????
				...
				CLEAR_BIT(I2C->CR1, I2C_CR1_ACK); //???? ?? ?????, ??? ????????? ???????? ???? ????? ?????????, ?? ???????? NACK
...
bool I2C_MemWrite(I2C_TypeDef *I2C, uint8_t Adress_Device, uint16_t Adress_data, uint8_t Size_adress, uint8_t* data, uint16_t Size_data, uint32_t Timeout_ms) {
	...
	if (READ_BIT(I2C->SR1, I2C_SR1_ADDR)) {
		...
		for (uint16_t i = 0; i < Size_adress; i++) {
			I2C->DR = *((uint8_t*)&Adress_data + (Size_adress - 1 - i)); //?????? ?????
			while (READ_BIT(I2C->SR1, I2C_SR1_TXE) == 0) { 
				//????, ???? ?????? ?????????? ? ??????? ??????.
